...
OpenKara's Dropbox integration is a desktop OAuth flow for remote library
storage. It stores OpenKara-created karaoke library files in a Dropbox folder
and syncs the local portable library database, imported media, CD+G files, and
...
remote library folder, `.openkara-library`, `openkara.db`, media, and stem files
already exist, and to read revision metadata for sync conflict checks.

`files.content.read` is used with `/2/files/download` to download the remote
library database, original audio files, CD+G graphics, and processed stem files
into the local working copy when opening or playing a remote library.
...
publish `openkara.db`, upload imported media and processed stems, and remove
library files when the user deletes them from OpenKara.

## Review Smoke Test

1. Install and launch OpenKara.
2. On first launch, create a local karaoke library in any writable folder.
3. Open Settings, choose Remote Library, select Dropbox, and click Connect
   Dropbox.
4. Complete the Dropbox authorization flow in the browser.
5. Return to OpenKara and create a new Dropbox remote library.
6. Import a small local audio file.
7. Publish or sync the library to the Dropbox-backed remote library.
8. Close and reopen OpenKara, open the same Dropbox remote library, and confirm
