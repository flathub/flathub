...
static void my_application_activate(GApplication* application) {
  ...
  for (int i = 0; i < 4; i++) {
    GdkPixbuf* icon = gtk_icon_theme_load_icon(theme, "hoptodesk", icons[i], GTK_ICON_LOOKUP_NO_SVG, NULL);
    if (icon != nullptr) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "hoptodesk");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "hoptodesk");
  }
