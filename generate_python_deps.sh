#! /bin/bash
# This create some dependencies that is not generate
python3 flatpak-pip-generator setuptools_rust hatchling rtree -o spyder_additional_deps &&
...
# Move python lib that requires rust to spyder_deps_rust.txt
...
# rm -f spyder_*.txt || true &&
# Build
flatpak run org.flathub.flatpak-external-data-checker --edit-only *.yaml
